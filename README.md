...
Weak auth usage — logins using basic username/password instead of SSO, key-based, or token-based authentication
Repeated failed logins — users with 5+ failed login attempts, which could indicate a locked-out account or someone attempting unauthorized access
Odd-hour logins — logins occurring outside normal working hours (before 6am or after 9pm)
Unusual system access — a breakdown of how often each user accesses each system, used to identify low-frequency, potentially unusual access patterns
...
I also looked at building this directly against a real Snowflake trial account, querying its built-in `LOGIN_HISTORY` system view instead of using generated data — since that would reflect exactly how a real security/data team would approach this on Snowflake itself. I decided to keep this project on fake, locally-generated data in order to hit a tight deadline, but a real-Snowflake version is a natural next iteration of this project (see "What I'd do next" below).
---
Tech used
...
Project structure
```
├── data/               # fake login dataset (CSV)
├── sql/                # the 4 SQL flag queries
├── screenshots/        # dashboard screenshot
├── app.py              # Streamlit dashboard
├── logins.db           # SQLite database
└── notes.txt           # working notes from building this project
```
---
What I'd do next
Rebuild this against a real Snowflake trial account, querying `SNOWFLAKE.ACCOUNT_USAGE.LOGIN_HISTORY` instead of generated data
